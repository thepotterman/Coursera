...
vector<Spending> LoadFromXml(istream& input) {
    vector<Spending> answer;
    Document doc = Load(input);
    string category;
    int spend;
    for(const Node & x : doc.GetRoot().Children()) {
        category = x.AttributeValue<string>("category");
        spend = x.AttributeValue<int>("amount");
        answer.push_back({category, spend});
    }
    return answer;
}
