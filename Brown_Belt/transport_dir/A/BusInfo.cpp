...
void BusInfo::ComputeRouteLength(const std::unordered_map<std::string, Coordinate>& stops) {
    for(int i = 1; i < stops_.size(); ++i) {
        long double current_distance = ComputeDistance(stops.at(stops_[i]), stops.at(stops_[i - 1]));
