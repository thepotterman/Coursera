...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        for(auto it = begin; it != end; ++it) {
            a[static_cast<size_t>(*it)].second++;
        }
    }
    
    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return a.at(static_cast<size_t>(airport)).second; 
    }
    
    // добавить данный элемент
    void Insert(TAirport airport) {
        ++a[static_cast<size_t>(airport)].second;
    }
    
    // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) {
        if(a[static_cast<size_t>(airport)].second == 0) {
    ...
    void EraseAll(TAirport airport) {
        a[static_cast<size_t>(airport)].second = 0;
    }
    ...
    array<Item, static_cast<size_t>(TAirport::Last_)> GetItems() const {
        return a;
    }
    
private:
    array<Item, static_cast<size_t>(TAirport::Last_)> a;
};
